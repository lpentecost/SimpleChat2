...
// INSTANCE METHODS *************************************************
  ...
  final public void listen() throws IOException
  {
    if (!isListening())
    {
      ...
      connectionListener.start(); // calls the run method. 
    }
  }
...
// RUN METHOD -------------------------------------------------------
  ...
  final public void run()
  {
    ...
    // This is implemented in EchoServer1
    serverStarted();
    ...
    try
    {
      ...
      while(!isReadyToStop())
      {
        try
        {
          ...
          synchronized(this)
          {
            ConnectionToClient c = new ConnectionToClient(this.clientThreadGroup, clientSocket, this); // this is the server
          }
        }
...
// METHODS DESIGNED TO BE OVERRIDDEN BY CONCRETE SUBCLASSES ---------
   ...
   *  sent the message.
   */
  protected abstract void handleMessageFromClient(Object msg, ConnectionToClient client); // implemented in EchoServer1
